<template>
  <div class="min-h-screen bg-[#FAF9F6] text-[#1A1A1A] font-sans antialiased pb-24">
    ...
    <!-- 2026 Studio SaaS Master Control HUD Header -->
    ...
    <main class="max-w-7xl mx-auto px-6 mt-8 space-y-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <section class="lg:col-span-8 space-y-4">
          ...
          <!-- Master Calendar Grid Container -->
          <div class="bg-white border border-[#1A1A1A]/10 rounded-none overflow-hidden shadow-sm">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 border-b border-[#1A1A1A]/10 bg-[#FAF9F6]">
              ...
              <div class="flex items-center gap-1">
                ...
                <div class="flex items-center border border-[#1A1A1A]/10 p-0.5 bg-white ml-2">
                  <button @click="navigateMonth(-1)" class="p-1 px-2.5 hover:bg-neutral-100 text-xs font-bold font-mono">◀</button>
            ...
            <div class="grid grid-cols-7 border-b border-[#1A1A1A]/10 bg-[#FAF9F6] text-center py-2">
              <span v-for="day in DAYS_OF_WEEK" :key="day" class="text-[9.5px] font-bold uppercase tracking-[0.2em] text-[#1A1A1A]/60">
                {{ day }}
              </span>
            </div>
            ...
            <div class="grid grid-cols-7 grid-rows-5 auto-rows-fr min-h-[580px] bg-neutral-100 divide-x divide-y divide-[#1A1A1A]/5">
              <div 
                ...
                :key="index"
                @click="onCellClick($event, cell.dateStr)"
                :class="[
    ...
    <!-- UNIFIED RESERVATION FORM MODAL -->
    <div v-if="isModalOpen" class="fixed inset-0 z-50 bg-[#1A1A1A]/50 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
        ...
        <div class="flex items-center justify-between border-b border-[#1A1A1A]/10 px-6 py-4 bg-[#FAF9F6]">
          <div>
            <h2 class="text-md font-serif font-bold italic text-[#1A1A1A] flex items-center gap-1.5 leading-none">
              {{ modalBookingId ? 'Edit Reservation' : 'Schedule New Workspace Session' }}
            </h2>
            <p class="text-[9.5px] text-neutral-400 mt-1 uppercase font-mono tracking-wider">
              ID: {{ modalBookingId || '(Auto Assigned BOK-' + activeTenant.plan.toUpperCase() + ')' }}
            </p>
          </div>
          <button @click="closeModal" class="p-1 px-2 hover:bg-neutral-100 border border-neutral-200 font-bold">×</button>
        </div>

        <!-- Scrollable Form Container -->
        <form @submit.prevent="saveBooking" class="p-6 overflow-y-auto flex-1 space-y-5 text-xs text-[#1A1A1A]">
          
          <!-- SaaS Booking Limit warning check -->
          <div v-if="!modalBookingId && cycleBookingsCount >= activeTenant.max_monthly_bookings" class="p-3 bg-red-50 text-red-700 border border-red-200">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="space-y-2">
              ...
              <div class="space-y-1 max-h-[140px] overflow-y-auto bg-white p-2 border border-[#1A1A1A]/10">
                <div v-if="filteredEmployees.length === 0" class="text-[10px] text-neutral-400 italic font-mono p-1">No active workspace staff loaded.</div>
                <button 
                  v-for="emp in activeEmployees" 
                  :key="emp.id"
                  type="button"
                  @click="toggleFormEmployee(emp.id)"
                  :class="[
            ...
            <div class="space-y-2">
              ...
              <div class="space-y-1 max-h-[140px] overflow-y-auto bg-white p-2 border border-[#1A1A1A]/10">
                <div v-if="filteredEquipment.length === 0" class="text-[10px] text-neutral-400 italic font-mono p-1">No active equipment items logged.</div>
                <button 
                  v-for="eq in activeEquipment" 
                  :key="eq.id"
                  type="button"
                  @click="toggleFormEquipment(eq.id)"
                  :class="[
                    'w-full p-1.5 border text-left flex justify-between items-center text-[10.5px] cursor-pointer',
        ...
        <div class="border-t border-[#1A1A1A]/10 p-4 bg-[#FAF9F6] flex items-center justify-between shrink-0">
          <div>
            <button 
              v-if="modalBookingId" 
              type="button" 
              @click="deleteBooking(modalBookingId)"
          ...
          <div class="flex items-center gap-2">
            <button @click="closeModal" type="button" class="px-4 py-2 border border-neutral-350 bg-white cursor-pointer hover:bg-neutral-50">
              Close
            </button>
            <button 
              v-if="modalBookingId || cycleBookingsCount < activeTenant.max_monthly_bookings"
              @click="saveBooking" 
              type="button" 
...
// --- RESERVATION FLOW ---
const onCellClick = (event, dateStr) => {
...
<style>
/* Classic typography & colors to pair beautifully with main layout definitions if necessary */
</style>
